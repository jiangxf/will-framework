...
public abstract class AQConsumer extends AbstractContextLoadListener {
    ...
    protected AtomicInteger countAvg = new AtomicInteger(0);
    ...
    public final int computeParallelCount() {
        ...
        countAvg.set(size / shedulerWaitS.get());
        if (size > 20) {
            size = 20;
        }
    ...
    public AtomicInteger getCountAvg() {
        return countAvg;
    }

    public void setCountAvg(AtomicInteger countAvg) {
        ...
        protected void doRun() {
            while (true) {
                ...
                if (queueLength <= 0) {
                    ...
                    int need = (int) ((queueLength + parallelCount + aqConsumer.getCountAvg().get()) / parallelCount);
                    real = need;
                ...
                if (queueLength > 0) {
                    String logInfo = "AQScheduler <> "
                            ...
                            aqConsumer.getAction(), queueLength, aqConsumer.getCountAvg().get(), aqConsumer.getIntervalAVG().get(),
                            parallelCount, aqConsumer.getAQProcessorMap().size(), aqConsumer.getNeedThread().get(), real
                    );
                    logger.info(logInfo);
                    //LogCollectManager.common(logInfo, "aq", aqConsumer.getAction());
                }
